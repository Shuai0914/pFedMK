...
class clientMK:
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        ...
        self.model = Ensemble(
            ...
            head_hp = copy.deepcopy(self.model.head)
        )
    ...
    def set_head_hp(self, head, weights):
        for new_param, old_param in zip(head.parameters(), self.model.head_hp.parameters()):
            old_param.data = new_param.data.clone()
        self.model.head_hp.load_state_dict(weights, strict=False)
    ...
    def generate_upload_head(self):
        for (np, pp), (ng, pg), (ny, py) in zip(self.model.model.head.named_parameters(),
                                                self.model.head_g.named_parameters(),
                                                self.model.head_hp.named_parameters()):
            pg.data = pp * 1 / 3 + pg * 1 / 3 + py * 1 / 3
    ...
    def train_cs_model(self):
        ...
        self.model.train()
        params_g1 = copy.deepcopy(list(self.model.head_hp.parameters()))
        params_mean1 = [torch.mean(p.data) for p in params_g1]
        # flat_params_g1 = torch.cat([p.data.view(-1) for p in params_g1])
        # global_mean1 = torch.mean(flat_params_g1)
        # global_mean_params_g1 = [torch.full_like(p, global_mean1) for p in params_g1]
        params1 = list(self.model.head_hp.parameters())
        params_p1 = params1[-2:]
        params_gp3 = params_g1[-2:]
        params_gp4 = params_mean1[-2:]
        weights_p1 = [param.data.to(self.device) for param in params_p1]
        params_g = copy.deepcopy(list(self.model.model.parameters()))
        # flat_params_g = torch.cat([p.data.view(-1) for p in params_g])
        # global_mean = torch.mean(flat_params_g)
        # global_mean_params_g = [torch.full_like(p, global_mean) for p in params_g]
        params_mean = [torch.mean(p.data) for p in params_g]
        params = list(self.model.model.parameters())
        params_p = params[-2:]
        params_gp1 = params_g[-2:]
        params_gp2 = params_mean[-2:]
        weights_p = [param.data.to(self.device) for param in params_p]

        for param in self.model.model.base.parameters():
        ...
        for _ in range(self.local_steps):
            for i, (x, y) in enumerate(trainloader):
                ...
                loss.backward()
                self.opt.step()
                for param, param_g1, param_g2, weight in zip(params_p, params_gp1, params_gp2, weights_p):
                    weight.data = torch.clamp(
                        weight - 0.01 * (param.grad * (param_g1 - param)) - 0.08 * (param.grad * (param_g2 - param)), 0,
                        1)
                for param, param_g1, param_g2, weight in zip(params_p, params_gp1, params_gp2, weights_p):
                    param.data = param + (param_g1 - param) * weight + (param_g2 - param) * weight
                for param, param_g1, param_g2, weight in zip(params_p1, params_gp3, params_gp4, weights_p1):
                    weight.data = torch.clamp(
                        weight - 1 * (param.grad * (param_g1 - param)), 0,
                        1)
                for param, param_g1, param_g2, weight in zip(params_p1, params_gp3, params_gp4, weights_p1):
                    param.data = param + (param_g1 - param) * weight + (param_g2 - param) * weight
        ...
        for _ in range(5):
            ...
            for i, (x, y) in enumerate(trainloader):
                ...
                incorrect_x = x[incorrect_mask]
                #incorrect_y = y[incorrect_mask]
                correct_x = x[correct_mask]
                #correct_y = y[correct_mask]
                if incorrect_x.size(0) > 0 and correct_x.size(0) == 0: # Different datasets have different weights; if the batch size is sufficient, dynamic weights can be set to enhance training.
                    weighted_loss = 0.7 * self.loss(output, y)
                elif correct_x.size(0) > 0 and incorrect_x.size(0) == 0:
                    weighted_loss = 0.3 * self.loss(output, y)
                elif incorrect_x.size(0) > 0 and correct_x.size(0) > 0:
                    # output1 = self.model(incorrect_x, is_rep=False, context=self.context)
                    # output2 = self.model(correct_x, is_rep=False, context=self.context)
                    # weighted_loss = 0.7 * self.loss(output1, incorrect_y) + 0.3 * self.loss(output2, correct_y)
                    weighted_loss = self.loss(output, y)
                else:
                    weighted_loss = 0
                loss = weighted_loss
...
class Ensemble(nn.Module):
    def __init__(self, model, cs, head_g, feature_extractor, head_hp) -> None:
        super().__init__()
    ...
    def forward(self, x, is_rep=False, context=None):
        ...
        if self.flag == 0:
            ...
            output = self.model.head(rep_p) * 0.8 + self.head_g(rep_g) + 0.2 * self.head_hp(rep_p)
        elif self.flag == 1:
